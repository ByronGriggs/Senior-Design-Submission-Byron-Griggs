...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
 /**
  ...
  void onResult(BLEAdvertisedDevice advertisedDevice) {
    ...
    /*or(int i = 0; i< advertisedDevice.getServiceDataUUIDCount(); i++) {
      Serial.println(advertisedDevice.getServiceDataUUID().toString().c_str());
    }*/
    //the below line is the original
    //if (advertisedDevice.haveServiceUUID() && advertisedDevice.isAdvertisingService(serviceUUID)) {
      bool found = true;
      std::string name = "UART Service";
      for (int i = 0; i<name.length(); i++){
        if (advertisedDevice.toString().c_str()[i+6] != name[i]){
          //Serial.print("name is "); Serial.print(name[i]); Serial.print(" ");
          //Serial.print( "actual is "); Serial.print(advertisedDevice.toString().c_str()[i+6]); Serial.print(" ");
          found = false;
        }
        //Serial.println();
      }
