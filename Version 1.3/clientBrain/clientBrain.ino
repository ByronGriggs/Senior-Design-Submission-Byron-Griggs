...
class MyClientCallback : public BLEClientCallbacks {
  ...
  void onDisconnect(BLEClient* pclient) {
    ...
    //remove server from list (probably causes memory leak)
...
//Connect to the remove BLE Server.
  ...
  for (std::map<std::string, BLERemoteCharacteristic*>::iterator it=serverMap->begin(); it!=serverMap->end();++it){
    ...
    else if (it->second->readValue() == "Not moving"){
...
void customSleep(){
  ...
  while (true){
    ...
    for (int i=0; i<myServers.size();i++){
      if (myServers[i].accChar->readValue() == "Moving"){
        movingServers++;
      }
    }
    //check if the client is moving
    M5.IMU.getAccelData(&accX, &accY, &accZ);
    if (sqrt(pow(accX,2.0)+pow(accY,2.0)+pow(accZ,2.0))-1 >= 0.2){
